...
float * datMax; float * datMin; float * datScale; float * dat; float * myMean;
int * clusterCentres;
int * currentLabel; 
...
/* find the most central element for a cluster with index J.. */
int mostCentralElementForClusterJ( int j ){
  ...
  for( it=myMembers->begin(); it!=myMembers->end(); it++){
    int myMember = *it;
    for( i=0; i< nFields; i++){
      myMean[i] += (dat[ myMember*nFields + i]-datMin[i])*(datScale[i]);
    }
    nObs+=1.;
  }
  for(i=0; i<nFields; i++){
...
void writeLabels(){
  ...
  if( !f ){ cout<<"Error: could not open label.bin for writing."<<endl; exit(1);}
  int i;
  for(i=0; i< nClust; i++){
...
int main(int argc, char ** argv){
  if( argc <2){ printf("kmeans.cpp: usage:\n\t./kmeans [infile] [k]\n"); exit(1); }
  std::ifstream f;  
  f.open( argv[1] );
  if(!f){ cout<<"Error: could not open file: "<< argv[1] <<endl; exit(1); }

  /* number of clusters to determine.. */
  nClust = 3; if( argc>=3){ nClust = atoi( argv[2] ); }
  ...
  nFields = nRecords = 0; long int li;
  datMax= datMin= datScale= dat= NULL; li=-1;
  ...
  for(it=lines.begin(); it!=lines.end(); it++){
    ...
    if(it==lines.begin()){
      ...
      dat = alloc( nFields * nRecords); datScale = alloc(nFields);
      datMin = alloc( nFields); datMax = alloc( nFields);
      int j;
      for(j=0; j<nFields; j++){
